...
function toBinaryKafkaMessage(event) {
    ...
    const headers = Object.assign({ [__1.CONSTANTS.HEADER_CONTENT_TYPE]: event.datacontenttype }, headers_1.headersFor(event));
    return {
        headers,
...
function deserializeKafkaMessage(message) {
    ...
    const cleanHeaders = headers_2.sanitize(m.headers);
...
function isKafkaEvent(message) {
    var _a, _b;
    const headers = headers_2.sanitize(message.headers);
